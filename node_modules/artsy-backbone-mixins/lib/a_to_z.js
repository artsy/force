  ...
  module.exports = {
    ...
    groupByAlphaWithColumns: function(numberOfColumns) {
      ...
      itemsToColumns = function(items, numberOfColumns) {
        var i, maxRows, _i, _results;
        maxRows = Math.floor(items.length / numberOfColumns);
        _results = [];
        for (i = _i = 0; 0 <= numberOfColumns ? _i < numberOfColumns : _i > numberOfColumns; i = 0 <= numberOfColumns ? ++_i : --_i) {
          _results.push(items.slice(i * maxRows + i, +((i + 1) * maxRows + i) + 1 || 9e9));
        }
        return _results;
      };
