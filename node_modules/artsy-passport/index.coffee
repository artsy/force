...
signup = (req, res, next) ->
  request.post(opts.API_URL + '/api/v1/user').send(
    name: req.body.name
...
submitTwitterLastStep = (req, res, next) ->
  ...
  request.put("#{opts.API_URL}/api/v1/me").send(
...
artsyCallback = (username, password, done) ->
  request.get("#{opts.API_URL}/oauth2/access_token").query(
...
facebookCallback = (req, accessToken, refreshToken, profile, done) ->
  if req.user
    request.post("#{opts.API_URL}/api/v1/me/authentications/facebook").query(
...
twitterCallback = (req, token, tokenSecret, profile, done) ->
  if req.user
    request.post("#{opts.API_URL}/api/v1/me/authentications/twitter").query(
      oauth_token: token
