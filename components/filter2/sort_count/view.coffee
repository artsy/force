...
module.exports = class FilterSortCount extends Backbone.View
  ...
  initialize: (options) ->
    _.extend @, options
    @collection.on 'sync', @renderTotal
    new BorderedPulldown el: @$('.bordered-pulldown')

  renderTotal: =>
    @$('.filter-sort-count-total').text _s.numberFormat(@collection.counts.total.value) + ' ' +
      if @collection.counts.total.value is 1 then 'Work' else 'Works'
