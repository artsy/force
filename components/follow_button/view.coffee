...
module.exports = class FollowButton extends Backbone.View
  ...
  toggle: (e) ->
    ...
    if @following.isFollowing @model.id
      ...
      analytics.track.click @analyticsUnfollowMessage, label: analytics.modelNameAndIdToLabel(@modelName, @model.get('id'))
    else
      @following.follow @model.id, notes: (@notes or @analyticsFollowMessage)
      analytics.track.click @analyticsFollowMessage, label: analytics.modelNameAndIdToLabel(@modelName, @model.get('id'))
      # Delay label change
