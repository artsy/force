...
module.exports = class ContactView extends ModalView
  ...
  events: -> _.extend super,
    'submit form'                : 'submit'
    'click #contact2-submit'      : 'submit'
    'mouseenter #contact2-submit' : 'logHover'
  ...
  renderTemplates: ->
    @$('#contact2-header').html @headerTemplate(@templateData)
    @$('#contact2-form').html @formTemplate(@templateData)

  success: =>
    @$dialog.attr 'data-state', 'fade-out'
    _.delay =>
      @$el.addClass 'contact2-success-message'
      @$dialog.
  ...
  submit: (e) ->
    ...
    if @validateForm (@$form ?= @$('form'))
      @$submit    ?= @$('#contact2-submit')
      @$errors    ?= @$('#contact2-errors')
