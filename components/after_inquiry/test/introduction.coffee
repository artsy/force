...
describe 'Introduction', ->
  ...
  describe 'logged out', ->
    ...
    it 'denotes the user as a collector', ->
      ...
      @introduction.blurb().should.equal 'Damon Zucconi is a collector.'

    it 'displays the user location', ->
      @user.set 'location', city: 'New York'
      @introduction.blurb().should.equal 'Damon Zucconi is based in New York.'

    it 'displays both collector and location', ->
      @user.set 'collector_level', 3
      @user.set 'location', city: 'New York'
      @introduction.blurb().should.equal 'Damon Zucconi is a collector based in New York.'
  ...
  describe 'logged in', ->
    beforeEach ->
      @user = new LoggedOutUser id: 'dzuc', name: 'Damon Zucconi', created_at: '2012-03-27T14:11:24Z'
      @bookmarks = new Bookmarks
      @introduction = new Introduction @user, @bookmarks

    it 'displays the bare minimum of information when the user is logged in', ->
      @introduction.blurb().should.equal 'Damon Zucconi has been an Artsy member since March 2012.'
    ...
    it 'denotes the user as a collector if they have a collector level of 3 or above', ->
      @user.set 'collector_level', 3
      @introduction.blurb().should.equal 'Damon Zucconi is a collector and has been an Artsy member since March 2012.'
    ...
    it 'displays the user collection if they have any', ->
      @bookmarks.newFromArtist new Backbone.Model(fabricate 'artist', name: 'Foo Bar')
      @bookmarks.newFromArtist new Backbone.Model(fabricate 'artist', name: 'Bar Baz')
      @introduction.blurb().should.equal 'Damon Zucconi has been an Artsy member since March 2012. Damon’s collection includes Bar Baz and Foo Bar.'

    it 'displays the user profession if present', ->
      @user.set 'profession', 'Engineer @ Artsy'
      @introduction.blurb().should.equal 'Damon Zucconi has been an Artsy member since March 2012. Damon’s profession is noted as “Engineer @ Artsy.”'
    ...
    it 'renders the correct string if the user has more than 3 artists in their collection (1)', ->
      ...
      @introduction.blurb().should
        .equal 'Damon Zucconi has been an Artsy member since March 2012. Damon’s collection includes Qux Whatever, Baz Qux, Bar Baz and 1 other artist.'
    ...
    it 'renders the correct string if the user has more than 3 artists in their collection (2)', ->
      ...
      @introduction.blurb().should
        .equal 'Damon Zucconi has been an Artsy member since March 2012. Damon’s collection includes Whatever Whatever, Qux Whatever, Baz Qux and 2 other artists.'
