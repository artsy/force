...
describe 'RelatedPostsView', ->
  ...
  describe 'default behavior', ->
    ...
    describe '#initialize', ->
      it 'makes the right API call', ->
        Backbone.sync.args[0][1].url.should.containEql 'api/v1/related/posts'
        Backbone.sync.args[0][2].data['artist[]'].should.equal @view.model.id

    describe '#render', ->
      it 'doesnt render anything if there are no results', ->
        Backbone.sync.args[0][2].success []
        @view.$el.html().should.containEql 'Do you have an interesting insight'
        @view.$el.html().should.containEql 'Add Post'
      ...
      it 'renders the right content', ->
        ...
        @view.$el.html().should.containEql 'is-list'
        @view.$el.html().should.containEql '<a href="/post/cats-rule-dogs-drool-literally" class="related-post-content">'
        @view.$el.html().should.containEql '<a href="/post/bitty-the-queen" class="related-post-content">'
      ...
      it 'correctly allows you to click see more', ->
        ...
        @view.$el.html().should.containEql '<a href="/post/cats-rule-dogs-drool-literally" class="related-post-content">'
        @view.$el.html().should.containEql '<a href="/post/bitty-the-queen" class="related-post-content">'
        @view.$el.html().should.containEql '<a href="/post/dogs-are-just-ok" class="related-post-content">'
      ...
      describe 'has 1 result', ->
        it 'properly pluralizes the headline', ->
          ...
          @view.$el.html().should.containEql '1 Post About Pablo Picasso'
      ...
      describe 'has 2 results', ->
        it 'properly pluralizes the headline', ->
          ...
          @view.$el.html().should.containEql '2 Posts About Pablo Picasso'
  ...
  describe 'non-default behavior', ->
    ...
    it 'can render a grid', ->
      ...
      @view.$el.html().should.containEql 'is-grid'
      @view.$el.html().should.containEql 'grid-4-up'
      @view.$el.html().should.containEql '<div class="rpc-author">By Craig Spaeth</div>'
      @view.$el.html().should.not.containEql 'Add Post'
    ...
    it 'can render extended previews', ->
      ...
      @view.$el.html().should.containEql 'is-extended'
      @view.$el.html().should.containEql 'By Craig Spaeth'
      @view.$el.html().should.not.containEql 'Add Post'
