...
import {
  ...
  PLACEHOLDER_XS,
  SuggestionItem,
} from "./Suggestions/SuggestionItem"
...
export class SearchBar extends Component<any, State> {
  ...
  renderSuggestionsContainer = ({ containerProps, children, query }) => {
    const noResults = this.props.viewer?.searchConnection?.edges?.length === 0
...
export const SearchBarQueryRenderer: React.FC<BoxProps> = props => {
  const isMounted = false //useDidMount(typeof window !== "undefined")

  if (!isMounted) {
    return <StaticSearchContainer searchQuery={""} {...props} />
  }
  ...
  return (
    <QueryRenderer
      ...
      render={({ props }) => {
        if (props) {
          return <SearchBarRefetchContainer viewer={(props as any).viewer} />
          ...
          return <StaticSearchContainer searchQuery={""} />
        }
      }}
    />
  )
