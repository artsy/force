...
describe('frameAnimator', () => {
  ...
  it('returns an animator function', () => {
    const animator = frameAnimator({}, () => {})
    const returnedType = typeof animator
    returnedType.should.equal('function')
  })

  describe('the returned animator function', () => {
    it('fires the callback within the requested duration', () => {
      const duration = 100
      const timestampWithinDuration = 99
      const callback = sinon.spy()

      const animator = frameAnimator({ duration }, callback)
      animator(timestampWithinDuration)
    ...
    it('does not fire the callback after the duration is up', () => {
      ...
      const animator = frameAnimator({ duration }, callback)
      animator(timestampAfterDuration)
