...
describe('analytics middleware', () => {
  ...
  it('tracks a login', () => {
    ...
    store.dispatch({ type: 'UPDATE_USER', payload: { user: { id: 'sarah', email: 'sarah@test.com' }, accountCreated: false } })
  ...
  it('tracks a signup', () => {
    const store = createStore(reducers, applyMiddleware(analyticsMiddleware))
    store.dispatch({ type: 'UPDATE_USER', payload: { user: { id: 'sarah', email: 'sarah@test.com' }, accountCreated: true } })
  ...
  it('tracks an artist confirmed action', () => {
    const store = createStore(reducers, applyMiddleware(analyticsMiddleware))
    store.dispatch({ type: 'UPDATE_ARTIST_ID', payload: { artistId: 'andy-warhol' } })
  ...
  it('tracks an error on submission creation', () => {
    const store = createStore(reducers, applyMiddleware(analyticsMiddleware))
    store.dispatch({ type: 'SUBMISSION_ERROR', payload: { errorType: 'convection_create' } })
  ...
  it('tracks an error on submission completion', () => {
    const store = createStore(reducers, applyMiddleware(analyticsMiddleware))
    store.dispatch({ type: 'SUBMISSION_ERROR', payload: { errorType: 'convection_complete_submission' } })
  ...
  it('tracks a submission created with no assets', () => {
    const store = createStore(reducers, applyMiddleware(analyticsMiddleware))
    store.dispatch({ type: 'SUBMISSION_CREATED', payload: { submissionId: 123 } })
  ...
  it('tracks a submission completed with no assets', () => {
    const store = createStore(reducers, applyMiddleware(analyticsMiddleware))
    store.dispatch({ type: 'UPDATE_SUBMISSION', payload: { submission: { id: 123 } } })
