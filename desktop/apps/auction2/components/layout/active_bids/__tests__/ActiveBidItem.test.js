...
describe('auction/components/layout/active_bids/ActiveBidItem.test', () => {
  describe('<ActiveBidItem />', () => {
    ...
    it('renders a desktop highest bid', () => {
      ...
      wrapper.find('.auction2-active-bid-item__current-bid').length.should.eql(1)
      wrapper.find('.auction2-active-bid-item__current-bid').text().should.containEql(`Current Bid: ${bid.sale_artwork.highest_bid.display}`)
    })
    ...
    it('renders a mobile highest bid', () => {
      ...
      wrapper.find('.auction2-active-bid-item__current-and-bids').length.should.eql(1)
      wrapper.find('.auction2-active-bid-item__current-bid').text().should.containEql(bid.sale_artwork.highest_bid.display)
    })
