...
const {
  ...
  REQUEST_EXPIRE_MS,
  REQUEST_LIMIT,
  SENTRY_PRIVATE_DSN_STAGING,
  SENTRY_PRIVATE_DSN_PRODUCTION,
  SESSION_COOKIE_KEY,
...
export default function (app) {
  ...
  let ravenDSN
  if (NODE_ENV === 'staging') {
    ravenDSN = SENTRY_PRIVATE_DSN_STAGING
  } else if (NODE_ENV === 'production') {
    ravenDSN = SENTRY_PRIVATE_DSN_PRODUCTION
  }

  RavenServer.config(ravenDSN).install()
  ...
  app.use(RavenServer.errorHandler()) // Raven error handling must come before other error middleware
  ...
  app.use((err, req, res, next) => {
    ...
    res.status(code).render(file, { message, stack: err.stack, code })
  })
}
