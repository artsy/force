  ...
  _CurrentUser = class CurrentUser extends User {
    ...
    initializeDefaultArtworkCollection(options) {
      if (!(__guard__(this.get("artworkCollections"), x => x.length) > 0)) {
        this.set({
          artworkCollections: [
            new ArtworkCollection({ userId: this.get("id") }),
          ],
        })
      }
      if (!this.defaultArtworkCollection.fetched) {
        return this.defaultArtworkCollection().fetch(options)
      }
    }
    ...
    defaultArtworkCollection() {
      if (__guard__(this.get("artworkCollections"), x => x[0]) == null) {
        throw Error(
          "Must call CurrentUser#initializeDefaultArtworkCollection " +
    ...
    followingGenes(options) {
      ...
      return this.get("followGenes").fetchUntilEnd(
        _.extend({ url, data }, options)
      )
    }
    ...
    isChecked(attribute) {
      if (_.isBoolean(this.get(attribute)) && this.get(attribute)) {
        return true
      } else {
        return undefined
      }
    ...
    placeBid(params, options) {
      ...
      return new Backbone.Model(params).save(
        null,
        _.extend(options, {
          url:
            `${API_URL}/api/v1/me/bidder_position` +
      ...
      return new Backbone.Collection().fetch({
        ...
        success(bidders) {
          return typeof options.success === "function"
            ? options.success(
                (bidders != null ? bidders.length : undefined) > 0
              )
            : undefined
        },
        error(m, xhr) {
          if (
            xhr.responseText != null
              ? xhr.responseText.match("A user is required")
              : undefined
          ) {
    ...
    savedArtwork(artworkId, options) {
      ...
      return new Backbone.Collection().fetch({
        url: `${API_URL}/api/v1/collection/saved-artwork/artworks?artworks[]=${artworkId}&private=true&user_id=${this.get(
          "id"
        )}`,
        success(artworks) {
          return typeof options.success === "function"
        ...
        error(m, xhr) {
          if (
            xhr.responseText != null
              ? xhr.responseText.match("Collection not found")
              : undefined
          ) {
    ...
    fetchQualifiedBidder(auctionId, options) {
      return new Backbone.Collection().fetch({
        ...
        success(bidders) {
          ...
          return typeof options.success === "function"
            ? options.success(
                bidder != null ? bidder.get("qualified_for_bidding") : undefined
              )
            : undefined
        },
      })
      ...
      return new Backbone.Collection().fetch({
        ...
        success: bidders => {
          const bidder = bidders.find(
            b => b.get("sale").id === auction.get("id")
          )
          if (!bidder) {
    ...
    followArtist(id, options) {
      return new Following(null, { kind: "artist" }).follow(
        id,
        _.extend(options, { access_token: this.get("accessToken") })
      )
    }
    ...
    followingArtists(options) {
      ...
      return this.get("followArtists").fetchUntilEndInParallel(
        _.extend({ url, data }, options)
      )
    }
    ...
    static orNull() {
      if (sd.CURRENT_USER) {
        return new this(sd.CURRENT_USER)
      } else {
        return null
      }
    }
    ...
    follow(id, kind, options) {
      return new Following(null, { kind }).follow(
        id,
        _.extend(options, { access_token: this.get("accessToken") })
      )
    }
