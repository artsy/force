...
describe('lifecycle', function() {
  ...
  beforeEach(function() {
    ...
    lifecycle.__set__('opts', (this.opts = {
      loginPagePath: '/login',
  ...
  describe('#onLocalLogin', function() {
    ...
    context('when erroring', function() {
      ...
      it('redirects invalid passwords to login', function() {
        this.err = { response: { body: { error_description: 'invalid email or password' } } };
        lifecycle.onLocalLogin(req, res, this.next);
  ...
  describe('#onLocalSignup', function() {
    ...
    it('sends 500s as json for xhr requests', function() {
      req.xhr = true;
      this.err = { response: { body: { message: 'Password must include at least one lowercase letter, one uppercase letter, and one digit.' } } };
      lifecycle.onLocalSignup(req, res, this.next);
      request.end.args[0][0](this.err);
      this.send.args[0][0].error.should.equal('Password must include at least one lowercase letter, one uppercase letter, and one digit.');
    });
  ...
  describe('#afterSocialAuth', function() {
    ...
    it('surfaces blocked by facebook errors', function() {
      passport.authenticate.returns((req, res, next) => next(new Error('Unauthorized source IP address')));
      lifecycle.afterSocialAuth('facebook')(req, res, this.next);
      res.redirect.args[0][0]
    ...
    it('passes random errors to be rendered on the login screen', function() {
      passport.authenticate.returns((req, res, next) => next(new Error('Facebook authorization failed')));
      lifecycle.afterSocialAuth('facebook')(req, res, this.next);
      res.redirect.args[0][0]
