...
module.exports = () => {
    ...
    client.putFile(pathname, remotePathname, headers, (err, res) => {
      if (err) {
        ...
        if (res.statusCode === 200) {
          log(
            `Uploaded heap dump to https://${S3_BUCKET}.s3.amazonaws.com/${S3_BUCKET}${remotePathname}`
          )
        } else {
  ...
  memwatch.on('leak', ({ growth }) => {
    ...
    log(
      `A leak was detected between ${start} and ${end} which grew the heap by ${growth} bytes`
    )
