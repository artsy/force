  ...
  return (
    <html lang="en">
      <head>
        ...
        <link rel="preconnect" href={imageCdnUrl} crossOrigin="anonymous" />
        <link rel="preconnect" href={fontUrl} crossOrigin="anonymous" />

        {/* Preload fonts */}
        <link rel="preload" href={`${fontUrl}/all-webfonts.css`} as="style" />
        <link
          rel="preload"
          href={`${fontUrl}/ll-unica77_regular.woff2`}
          as="font"
          type="font/woff2"
        ...
        <link
          rel="preload"
          href={`${fontUrl}/ll-unica77_medium.woff2`}
          as="font"
          type="font/woff2"
          crossOrigin="anonymous"
        />
        <link
          rel="preload"
          href={`${fontUrl}/ll-unica77_italic.woff2`}
          as="font"
          type="font/woff2"
          crossOrigin="anonymous"
        />
        ...
        <link
          type="text/css"
          rel="stylesheet"
          href={`${fontUrl}/all-webfonts.css`}
        />

        {/* Conditional scripts */}
        {!disable.scripts &&
          htmlWebpackPlugin?.tags.headTags.map((originalTag, idx) => (
            <script
              key={idx}
              {...originalTag.attributes}
              src={`${cdnUrl}${originalTag.attributes.src}`}
            />
          ))}

        {/* Preload tags */}
        <div dangerouslySetInnerHTML={{ __html: content.linkPreloadTags }} />
        <div dangerouslySetInnerHTML={{ __html: content.scripts }} />
      ...
      <body>
        ...
        <div id="root" dangerouslySetInnerHTML={{ __html: content.body }}></div>

        {/* Body scripts */}
        {!disable.scripts &&
          htmlWebpackPlugin?.tags.bodyTags.map((originalTag, idx) => (
            <script
              key={idx}
              {...originalTag.attributes}
              src={`${cdnUrl}${originalTag.attributes.src}`}
        ...
        {/* Segment script */}
        {!disable.segment &&
          !sd.THIRD_PARTIES_DISABLED &&
          sd.SEGMENT_WRITE_KEY && (
            <script
