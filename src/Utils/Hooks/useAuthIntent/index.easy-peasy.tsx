...
interface AuthIntentStoreModel {
  // State
  value: AfterAuthAction | null

  // Actions
  setValue: Action<AuthIntentStoreModel, AfterAuthAction | null>
  clearValue: Action<AuthIntentStoreModel>
}

// Create the context store
export const AuthIntentStore = createContextStore<AuthIntentStoreModel>(
  runtimeModel => ({
    // State
    value: runtimeModel?.value || null,
...
export const AuthIntentProvider: React.FC<React.PropsWithChildren<unknown>> = ({
  ...
  return (
    <AuthIntentStore.Provider>
      <AuthIntentProviderWrapper>{children}</AuthIntentProviderWrapper>
    </AuthIntentStore.Provider>
  )
}

// Internal wrapper to provide backward compatible context
const AuthIntentProviderWrapper: React.FC<React.PropsWithChildren<unknown>> = ({
  children,
}) => {
  const value = AuthIntentStore.useStoreState(state => state.value)
  const { setValue } = AuthIntentStore.useStoreActions(actions => actions)
...
export const useAuthIntent = () => {
  ...
  const { clearValue } = AuthIntentStore.useStoreActions(actions => actions)
