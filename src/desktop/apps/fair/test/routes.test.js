...
describe('Fair routes', function() {
  ...
  describe('#captureSignup', function() {
    ...
    it('follows the fair and adds a user fair action to the users collector profile', function() {
      ...
      this.req.params =
        {action: 'attendee'};
      this.req.user = new CurrentUser(fabricate('user', {accessToken: 'secret-code'}));
      routes.captureSignup(this.req, this.res, (next = sinon.stub()));
      ...
      return Backbone.sync.args[0][1].get('access_token').should.equal('secret-code');
    });
    ...
    return it('triggers next if the action is attendee', function() {
      ...
      this.req.user = new CurrentUser(fabricate('user'));
      return routes.captureSignup(this.req, this.res, (next = sinon.stub()))
  ...
  describe('#showRedirect', function() {
    ...
    return it('redirects to show page', function() {
      ...
      return this.res.redirect.args[0][0].should.containEql('/show/gagosian-gallery-inez-and-vinood');
    });
  });

  return describe('malformed /artworks/artworks urls', function() {
    beforeEach(() => sinon.stub(Backbone, 'sync'));

    afterEach(() => Backbone.sync.restore());

    return it('redirects to /:id/browse/artworks', function() {
      routes.malformedFilterRedirect(this.req, this.res);
      this.res.redirect.args[0][0].should.eql(301);
      return this.res.redirect.args[0][1].should.containEql('/some-fair/browse/artworks');
    });
  });
});
...
describe('#fetchFairData', function() {
  ...
  beforeEach(function() {
    ...
    const profile = new Profile(_.extend(fabricate('fair_profile'), { owner_type: 'Fair', id:'thefooshow' }));
...
describe('#fetchFairByOrganizerYear', function() {
  ...
  beforeEach(function() {
    ...
    const profile = new Profile(_.extend(fabricate('fair_organizer_profile'), {owner:fabricate('fair_organizer')}));
...
describe('microsite middleware', function() {
  ...
  return describe('has the microsite context', function() {
    ...
    return it('sets up the data for the microsite header template', function() {
      ...
      // Locals
      (fair = this.res.locals.micrositeFair).constructor.name.should.equal('Fair');
      ...
      (profile = this.res.locals.micrositeProfile).constructor.name.should.equal('Profile');
