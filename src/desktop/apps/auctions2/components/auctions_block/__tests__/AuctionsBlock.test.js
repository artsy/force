...
describe("auctions2/components/auctions_block/AuctionsBlock.test.js", () => {
  xdescribe("<AuctionsBlock />", () => {
    it("renders an auction item", () => {
      ...
      wrapper
        .html()
        .should.containEql('<a href="heritage-auctions-the-future-is-now"')
    ...
    describe("Date formatting", () => {
      it("Renders upcoming registration_ends_at date (hours)", () => {
        fixtures.currentAuctions[0].registration_ends_at = moment().add(
          3,
          "hours"
        )
        ...
        wrapper
          ...
          .should.containEql(
            "Register by " +
              moment(fixtures.currentAuctions[0].registration_ends_at).format(
                "ha"
              )
          )
      })
      ...
      it('Renders "In Progress" if live auction has started', () => {
        ...
        fixtures.currentAuctions[0].live_start_at = moment().subtract(
          20,
          "hours"
        )
        const { wrapper } = renderTestComponent({
