...
describe 'Fairs routes', ->
  beforeEach ->
    ...
    @currentFairs = _.times 2, ->
      fabricate('fair', image: image, profile: profile, id: _.uniqueId(), is_published: true, has_full_feature: true, has_listing: true, organizer: fabricate('fair_organizer'), end_at: moment().add(11, 'days').format(), banner_size: 'x-large')
    @pastFairs = _.times 4, ->
    ...
    @upcomingFairs = _.times 3, ->
      fabricate('fair', id: _.uniqueId('upcoming'), is_published: true, has_full_feature: true, has_listing: true, organizer: null, end_at: moment().add(11, 'days'))
    @invalidFairs = [
