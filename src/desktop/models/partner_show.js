...
export default _PartnerShow = (function () {
  _PartnerShow = class PartnerShow extends Backbone.Model {
    ...
    url() {
      if (this.has("partner")) {
        return `${sd.API_URL}/api/v1/partner/${
          this.get("partner").id
        }/show/${this.get("id")}`
      } else {
    ...
    formatArtistText() {
      ...
      if (artists.length > 1) {
        return (artistText = `${artists
          .slice(0, artists.length - 1)
          .join(", ")} and ${artists[artists.length - 1]}`)
    ...
    metaImageUrl() {
      if (this.imageUrl("large")) {
        ...
        (this.artworks != null ? this.artworks.length : undefined) > 0 &&
        this.artworks.first().defaultImage()
      ) {
    ...
    thumbImageUrl() {
      if (this.hasImage("general")) {
        return this.imageUrl("general")
      }
      if (this.hasImage("medium_rectangle")) {
        return this.imageUrl("medium_rectangle")
      }
    }
    ...
    shareTitle() {
      if (this.has("fair_location")) {
        return `${this.get("name")}, ${
          this.get("fair_location").display
        } See it on @artsy`
      } else if (this.has("partner")) {
        return `See \"${this.get("name")}\" at ${
          this.get("partner").name
        } on @artsy`
      } else {
    ...
    formatArtists(max) {
      ...
      const artists = this.get("artists").map(
      ...
      if ((artists != null ? artists.length : undefined) <= max) {
        ...
        return `${artists
          .slice(0, +(max - 1) + 1 || undefined)
          .join(", ")} and ${artists.slice(max - 1).length - 1} more`
      }
    }
    ...
    formatCity() {
      return __guard__(
        __guard__(this.get("location"), x1 => x1.city),
        x => x.trim()
      )
    }
    ...
    formatShowOrFairCity() {
      ...
      return city != null
        ...
        : (city = __guard__(
            __guard__(
              __guard__(this.get("fair"), x2 => x2.location),
              x1 => x1.city
            ),
            x => x.trim()
          ))
    ...
    formatStreetAddress() {
      return __guard__(
        __guard__(this.get("location"), x1 => x1.address),
        x => x.trim()
      )
    }
    ...
    formatFeedItemHeading(max) {
      ...
      if (__guard__(this.get("name"), x => x.length) > 0) {
        return this.get("name")
      }
    ...
    daySchedules() {
      return (
        __guard__(
          __guard__(this.location(), x1 => x1.get("day_schedules")),
          x => x.length
        ) > 0
      )
    }
    ...
    formatDaySchedule(day) {
      if (this.daySchedules()) {
        if (
          ...
          let hours
          const daySchedules = _.where(this.location().get("day_schedules"), {
            day_of_week: day,
          })
          if (daySchedules.length) {
            hours = []
            for (let daySchedule of Array.from(daySchedules)) {
              const startHour = moment().hour(
                daySchedule["start_time"] / 60 / 60
              )
              const startMinute = moment().minutes(
                daySchedule["start_time"] / 60
              )
...
${endHour.format("a")}`)
      ...
      if (this.daySchedules()) {
        ...
        _.each(this.formatDaySchedules().slice(1), function (daySchedule) {
          if (daySchedule["hours"] === _.last(daysOpen)["hours"]) {
            return _.extend(_.last(daysOpen), {
        ...
        return _.chain(daysOpen)
          ...
          .reject(day_schedule => day_schedule["hours"] === "Closed")
          .value()
      }
    }
    ...
    performers() {
      return Array.from(this.artists().models).map(artist =>
        artist.toJSONLDShort()
      )
    }
