...
export default(_Fair = (function() {
  _Fair = class Fair extends Backbone.Model {
    ...
    preinitialize() {
      this.aToZCollection = this.aToZCollection.bind(this);
    }
    ...
    profileId() {
      return this.get('default_profile_id') || __guard__(this.get('organizer'), x => x.profile_id);
    }

    fairOrgHref() {
      return `/${__guard__(this.get('organizer'), x => x.profile_id)}/${this.formatYear()}`;
    }

    hasImage(version) {
      let needle;
      if (version == null) { version = 'wide'; }
      return (needle = version, Array.from((this.get('image_versions') || [])).includes(needle));
    }
    ...
    profileImage(version){
      if (version == null) { version = 'square140'; }
      let url = `${sd.API_URL}/api/v1/profile/${this.get('default_profile_id')}/image/${version}`;
      if (sd.ARTSY_XAPP_TOKEN != null) { url = `${url}?xapp_token=${sd.ARTSY_XAPP_TOKEN}`; }
      return url;
    }
    ...
    fetchExhibitors(options) {
      const galleries = new this.aToZCollection('show', 'partner');
      return galleries.fetchUntilEnd({
        ...
        success() {
          ...
          return (options != null ? options.success(aToZGroup, galleries) : undefined);
        },
        error() {
    ...
    fetchArtists(options) {
      ...
      const artists = new this.aToZCollection('artist');
    ...
    fetchSections(options) {
      ...
      return sections.fetch(_.extend(options, {
        cache: true,
        cacheTime: DEFAULT_CACHE_TIME,
    ...
    fetchPrimarySets(options) {
      ...
      return orderedSets
        .fetchItemsByOwner('Fair', this.get('id'), {cache: options.cache, cacheTime: options.cacheTime})
        .done(function() {
          for (let set of Array.from(orderedSets.models)) {
            const items = set.get('items').filter(item => item.get('display_on_desktop'));
    ...
    aToZCollection(namespace) {
      let FairSearchResults;
      const href = this.href();
      class FairSearchResult extends Backbone.Model {
        href() {
          if ((namespace === 'show') && __guard__(this.get('partner_show_ids'), x => x[0])) {
            return `/show/${this.get('partner_show_ids')[0]}`;
          } else {
            return `${href}/browse/${namespace}/${this.get('id')}`;
          }
        }
        ...
        imageUrl() {
          let url = `${sd.API_URL}/api/v1/profile/${this.get('default_profile_id')}/image`;
          if (sd.ARTSY_XAPP_TOKEN != null) { url = url + `?xapp_token=${sd.ARTSY_XAPP_TOKEN}`; }
          return url;
        }
      }
      return new ((FairSearchResults = (function() {
        FairSearchResults = class FairSearchResults extends Profiles {
          static initClass() {
            this.prototype.model = FairSearchResult;
          }
          ...
          groupByColumns(columnCount) {
            ...
            const columns = [];
            for (let n = 0, end = columnCount, asc = 0 <= end; asc ? n < end : n > end; asc ? n++ : n--) {
              columns.push(this.models.slice(n*itemsPerColumn, +(((n+1)*itemsPerColumn) - 1) + 1 || undefined));
            }
            return columns;
          }
        };
        FairSearchResults.initClass();
        return FairSearchResults;
      })()));
    }
    ...
    itemsToColumns(items, numberOfColumns) {
      if (numberOfColumns == null) { numberOfColumns = 2; }
    ...
    filteredSearchColumns(filterdSearchOptions, numberOfColumns, key, namespace) {
      if (numberOfColumns == null) { numberOfColumns = 2; }
      if (key == null) { key = 'related_gene'; }
      if (namespace == null) { namespace = 'artworks'; }
      const href = this.href();
      const items = (() => {
        const result = [];
      ...
      return this.fetch({
        ...
        success: () => {
          ...
          const after = _.after(3, () => {
            return options.success(_.extend(data, {
          ...
          this.fetchSections({error: options.error, success: x => { data.sections = x; return after(); }});
          this.fetchExhibitors({error: options.error, success: (x, y) => {
            data.exhibitorsAToZGroup = x;
            data.galleries = y;
            return after();
          }
          });
          return this.fetchArtists({error: options.error, success: x => { data.artistsAToZGroup = x; return after(); }});
        }
      });
    }

    isEligible() {
      return this.get('has_full_feature') &&
      this.get('published') &&
