...
export default (_Artwork = (function() {
  _Artwork = class Artwork extends Backbone.Model {
    ...
    bidSuccessUrl() { return `${this.href()}/confirm-bid`; }

    initialize() {
      const { Articles } = require('../collections/articles');
      this.relatedArticles = new Articles;
      return this.relatedArticles.url += `?artwork_id=${this.get('_id')}&published=true`;
    }

    parse(response, options) {
      this.editions = new Backbone.Collection(response != null ? response.edition_sets : undefined, {model: Edition});
      return response;
    }

    defaultImage() {
      return this.related().images.default() ||
      new AdditionalImage;
    }
    ...
    normalizedDimensions() {
      return _.map(this.dimensions({metric: 'cm', format: 'decimal'}).replace('cm', '').split(' × '), parseFloat);
    }
    ...
    // return {Boolean}
    isHangable() {
      if (__guard__(this.get('category'), x => x.match(/sculpture|installation|design/i))) { return false; }
      if (this.hasDimension('depth')) { return false; }
      if (this.hasDimension('diameter')) { return false; }
      if (this.hasDimension('width') && this.hasDimension('height') && !this.tooBig()) { return true; }
      return false;
    }

    isPartOfAuctionPromo() {
      return (this.related().sales.firstAuctionPromo() != null);
    }

    // Should we include a form or button to contact the partner?
    //
    isContactable() {
      if (this.isAtLimitedFairPartner()) { return false; }
      if (this.get('inquireable') || this.get('is_inquireable')) { return true; }
      if (this.isPartOfContactableAuctionPromo()) { return true; }
      if (this.isPartOfAuction()) { return false; }
      return this.isArtworkContactable();
    }

    isPartOfAuction() {
      return (this.related().sales.firstAuction() != null);
    }
    ...
    isPartOfContactableAuctionPromo() {
      let auctionPromo;
      return ((auctionPromo = this.related().sales.firstAuctionPromo()) != null) && auctionPromo.isPreview();
    }
    ...
    // return {Boolean}
    hasMoreInfo() {
      return !_.isEmpty(this.get('blurb')) ||
    ...
    contactLabel() {
      if (__guard__(this.get('partner'), x => x.type) === 'Gallery') { return 'gallery'; } else { return 'seller'; }
    }
    ...
    priceLabel() {
      if (this.get('sold')) { return 'Sold'; } else { return 'Price:'; }
    }
    ...
    priceDisplay() {
      if ((this.get('availability') === 'for sale') && this.get('price_hidden')) {
        ...
        return this.get('price') || ((this.get('sale_message') !== 'Sold' ? this.get('sale_message') : undefined));
      }
    }
    ...
    isAtLimitedFairPartner() {
      return this.has('partner') && this.get('partner').has_limited_fair_partnership;
    }
    ...
    editionStatus() {
      if (this.get('unique')) {
        return 'Unique';
      } else if ((this.editions != null ? this.editions.length : undefined) === 1) {
    ...
    partnerLink() {
      ...
      if (!partner || (partner.type === 'Auction')) { return; }
      if (partner.default_profile_public && partner.id) {
        return `/partner/${partner.id}`;
      }
    }

    partnerLinkTarget() {
      if (this.get('partner').linkType() === 'external') { return '_blank'; } else { return '_self'; }
    }

    getTitle() {
      if (this.get('title')) { return this.get('title'); } else { return '(Untitled)'; }
    }

    titleAndYear() {
      return _.compact([
        (this.get('title') != null) && (this.get('title').length > 0) ? `<em>${this.get('title')}</em>` : '',
        this.get('date')
      ]).join(", ");
    }
    ...
    toAltText() {
      return _.compact([
        ...
        (this.get('partner') ? `, ${__guard__(this.get('partner'), x1 => x1.name)}` : undefined)
      ]).join('');
    }
    ...
    partnerDescription() {
      if (!__guard__(this.get('partner'), x => x.name)) { return undefined; }
      if (this.get('forsale')) { return `Available for sale from ${this.get('partner').name}`; } else { return `From ${this.get('partner').name}`; }
    }
    ...
    saleMessage() {
      if (this.get('availability_hidden')) { return; }
      if (this.get('sale_message') === 'Contact For Price') { return; }
      if ((this.get('availability') === 'not for sale') || (this.get('availability') === 'permanent collection')) { return; }
    ...
    availabilityMessage() {
      if (this.get('availability_hidden')) { return; }
      if (__guard__(this.get('partner'), x => x.type) === "Institutional Seller") { return; }
      if ((this.get('availability') === 'for sale') || (this.get('availability') === 'not for sale') || (this.get('availability') === 'permanent collection')) { return; }
      if (__guard__(this.get('partner'), x1 => x1.has_limited_fair_partnership)) {
        return 'Not inquireable';
      } else if (__guard__(this.get('availability'), x2 => x2.indexOf('on hold')) > - 1) {
        return _.compact([
          'On hold',
          this.get('price')
        ]).join(' - ');
    ...
    noPinAttr() {
      if (this.get('can_share_image')) { return undefined; } else { return "nopin"; }
    }
    ...
    showActionsList(user) {
      return this.get('website') || this.isDownloadable() || (user && user.isTeam());
    }
    ...
    artistName() {
      return this.related().artist.get('name') || _.compact(this.related().artists.pluck('name'))[0] || '';
    }

    artistsNames() {
      const names = _.compact(this.related().artists.pluck('name'));
      if (!(names.length > 1)) { return this.artistName(); }
