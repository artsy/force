  ...
  _User = class User extends Backbone.Model {
    ...
    isAttending(fair) {
      return this.related()
        ...
        .userFairActions.isAttending(fair)
    }
    ...
    static instantiate(attributes) {
      ...
      return CurrentUser.orNull() || new LoggedOutUser(attributes)
    }

    prepareForInquiry() {
      return this.findOrCreate({ silent: true }).then(() => {
        return Promise.all([
          new Promise(resolve => {
            if (this.isLoggedIn()) {
              return resolve()
