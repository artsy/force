...
export default _Artwork = (function () {
  _Artwork = class Artwork extends Backbone.Model {
    ...
    setImagesCollection(images) {
      return (this.images = new Backbone.Collection(images, {
    ...
    defaultImage() {
      ...
      return (
        this.images().findWhere({ is_default: true }) ||
        __guardMethod__(this.images, "first", o => o.first()) ||
        new AdditionalImage()
      )
    }
    ...
    fetchRelatedArtworks(options) {
      ...
      artworks.url =
        `${sd.API_URL}/api/v1/related/layer/` +
    ...
    showPriceLabel() {
      return (
        (__guard__(this.get("edition_sets"), x => x.length) <= 1 &&
    ...
    showNotForSaleLabel() {
      let needle
      return (
        this.get("inquireable") &&
        !((needle = this.get("availability")),
        ["sold", "for sale"].includes(needle))
      )
    }
    ...
    availableForSale() {
      ...
      return (
        (needle = this.get("availability")),
        !["not for sale", "sold", "on hold"].includes(needle)
      )
    }
