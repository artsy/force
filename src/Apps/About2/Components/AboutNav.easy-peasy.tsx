...
interface AboutNavStoreModel {
  // State
  active: Section | null
  visible: boolean

  // Actions
  activate: Action<AboutNavStoreModel, Section | null>
  hide: Action<AboutNavStoreModel>
  show: Action<AboutNavStoreModel>
}

// Create the context store
export const AboutNavStore = createContextStore<AboutNavStoreModel>(
  runtimeModel => ({
    // State
    active: runtimeModel?.active || null,
    visible: runtimeModel?.visible ?? true,

    // Actions
    activate: action((state, payload) => {
...
const AboutNavContext = createContext<{
  active: Section | null
...
export const AboutNavProvider = ({
  ...
  return (
    <AboutNavStore.Provider>
      <AboutNavProviderWrapper>{children}</AboutNavProviderWrapper>
    </AboutNavStore.Provider>
  )
}

// Internal wrapper to provide backward compatible context
const AboutNavProviderWrapper: React.FC<React.PropsWithChildren<unknown>> = ({
  children,
}) => {
  const state = AboutNavStore.useStoreState(state => state)
  const actions = AboutNavStore.useStoreActions(actions => actions)
