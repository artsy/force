...
type SavedAddressResult = Result<{ data: SavedAddressType }>
type DeleteAddressResult = Result<{ data: { deletedAddressID: string } }>
...
export const useUserAddressUpdates = () => {
  ...
  const handleNewUserAddressUpdates = async (
    ...
    let result: SavedAddressResult = null
    ...
    return handleUserAddressAction(userAddressAction)
  }

  const handleUserAddressAction = async (
    ...
    try {
      ...
      const addressID =
        userAddressAction.type === "create"
          ? result.data?.internalID
          : userAddressAction.addressID
      if (setAsDefault && userAddressAction.type !== "delete") {
        if (addressID) {
          const updateDefaultAddressResult = await updateDefaultAddress.submitMutation(
            {
              variables: {
                input: {
          ...
          // TODO: Handle errors in a special way here? Other operations are
          // already done, so we probably don't want to undo them
        } else {
          logger.error(
            "Expected address ID to exist when setting as default address after " +
              userAddressAction.type
          )
        }
      }
