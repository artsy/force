...
import { ChangeEmailValidator } from "../Authentication/Validators"
import { PasswordInput } from "../PasswordInput"
import { SystemContextProps } from "@artsy/reaction/dist/Artsy"
import { UpdateUserInformation } from "./UserInformation/UpdateUserInformationMutation"
...
export const UserInformation: React.FC<UserInformationProps> = ({ me, relay }) => {
  const { relayEnvironment } = useSystemContext()

  const onSubmit = async (
    { email, name, phone }: UserInformation_me,
    formikBag: FormikProps<any>
  ) => {
    try {
      const { updateMyUserProfile: { userOrError } } = await UpdateUserInformation(relayEnvironment, {
        email,
        name,
        phone,
      })
  ...
  return (
    <Box>
      <Serif size="6" mb={space(2)}>Information</Serif>
      <Formik
        initialValues={me}
