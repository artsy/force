...
module.exports = class PartnerView extends Backbone.View
  ...
  getDisplayableSections: (sections) ->
    criteria =
      ...
      works: => @currentUser?.isAdmin() and @partner.get('display_works_section')
      collection: => @partner.get('published_not_for_sale_artworks_count') > 0 and (if @currentUser?.isAdmin() then @partner.get('display_works_section') else true)
      contact: => true
      about: => true
      articles: => true
