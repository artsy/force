  ...
  query artist($artist_id: String!, $shows: Boolean!, $articles: Boolean!) {
    artist(id: $artist_id) {
      ... on Artist @include(if: $shows) {
        group_shows: shows(at_a_fair: false, solo_show:false, sort: start_at_desc, size: 99) {
          ... relatedShow
        }
        solo_shows: shows(at_a_fair: false, solo_show:true, sort: start_at_desc, size: 99) {
          ... relatedShow
        }
