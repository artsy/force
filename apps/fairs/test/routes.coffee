...
describe 'Fairs routes', ->
  beforeEach ->
    ...
    @currentFairs = _.times 2, ->
      fabricate('fair', image: image, profile: profile, id: _.uniqueId(), is_published: true, has_full_feature: true, organizer: fabricate('fair_organizer'), end_at: moment().add(10, 'days').format(), banner_size: 'x-large')
    @pastFairs = _.times 4, ->
      fabricate('fair', image: image, profile: profile, id: _.uniqueId('past'), is_published: true, has_full_feature: true, organizer: fabricate('fair_organizer'), end_at: moment().subtract(10, 'days').format())
    @upcomingFairs = _.times 3, ->
      fabricate('fair', id: _.uniqueId('upcoming'), is_published: true, has_full_feature: true, organizer: null, end_at: moment().add(10, 'days'))
    @invalidFairs = [
      fabricate('fair', id: _.uniqueId('invalid'), is_published: false)
      fabricate('fair', id: _.uniqueId('invalid'), is_published: true, has_full_feature: false)
    ]
